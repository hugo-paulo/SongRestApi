...
namespace SongRestApiIntergrationTest
{
    public class AlbumControllerTest : IntergrationTest
    {
        ...
        public async Task GetAllAlbumsTest()
        {
            ...
            //albums.Should().BeEquivalentTo(expectation, options => options.ComparingByValue<Album>()); //?This will fail when adding the options?
            albums.Should().BeEquivalentTo(expectation); //Tests will fail when comparing CustomObject, not primitives (need fluent package to make things easy to comparison)
